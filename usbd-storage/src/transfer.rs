...
#[cfg(feature = "imxrt-transfer")]
impl TransferBus for imxrt_usbd::BusAdapter {
    fn submit_write(&self, ep: EndpointAddress, buf: &[u8]) -> Result<(), UsbError> {
        imxrt_usbd::BusAdapter::submit_write(self, ep, buf)
    }

    fn submit_read(&self, ep: EndpointAddress, buf: &mut [u8]) -> Result<(), UsbError> {
        imxrt_usbd::BusAdapter::submit_read(self, ep, buf)
    }

    fn poll_transfer(&self, ep: EndpointAddress) -> Option<Result<usize, UsbError>> {
        imxrt_usbd::BusAdapter::poll_transfer(self, ep)
    }
}
